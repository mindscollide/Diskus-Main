...
import { getCommitteeMembersRole, getCommitteeTypes, updateCommittee } from "../../../store/actions/Committee_actions";
const UpdateCommittee = ({setUpdateComponentpage}) => {
  ...
  console.log(committeeData, "committeeDatacommitteeDatacommitteeDatacommitteeData")
  ...
  const handleAddAttendees = () => {
    let findRoleID = committeeMemberRolesOptions && committeeMemberRolesOptions.find((data, index) => data.label === participantRoleName);
    ...
    if (committeeMemberRolesOptions !== undefined && committeeMemberRolesOptions.length !== null) {
      ...
      if (committeeMemberRolesOptions.length > 0 && attendees.length === 0) {
        committeeMemberRolesOptions.map((data, index) => {
          console.log("datadatadatadatatest", data, participantRoleName.toString())
  ...
  useEffect(() => {
    if (CommitteeReducer.getCommitteeByCommitteeID !== null && CommitteeReducer.getCommitteeByCommitteeID !== undefined) {
      ...
      console.log("committeedetailscommitteedetailscommitteedetails", committeedetails)
      let newArr = [];
      let newData = [];
      let committeeID = 0;
      if (committeedetails.committeMembers.length > 0) {
        committeedetails.committeMembers.map((memberData, index) => {
          ...
          newArr.push({
            FK_UID: memberData.pK_UID, FK_CMMRID: memberData.committeeRole.committeeRoleID, FK_CMID: memberData.committeeID
          })
          if (meetingAttendeesList.length > 0) {
            meetingAttendeesList.map((data, index) => {
                        ...
                        <Col lg={8} md={8} sm={8}>
                          ...
                          <Row>
                            ...
                            >
                              ...
                              {/* Group Heads */}
                              <Row className="mt-2">
                                {groupMembers.filter((filterData, index) => filterData.role === 2).map((data, index) => {
                                  if (data !== null && data !== undefined) {
                                    return <Col lg={4} md={4} sm={12}>
                                      <Row>
                                        ...
                                        {createrID !== data.data.pK_UID ? <Col
                                          lg={2}
                                          md={2}
                                          sm={12}
                                          className="mt-0  d-flex justify-content-center"
                                        >
                                          <img src={CrossIcon} className="cursor-pointer" width={18} onClick={() => removeMemberHandler(data.data.pK_UID)} />
                                        </Col> : <Col
                                          lg={2}
                                          md={2}
                                          sm={12}
                              ...
                              <Row className="mt-2">
                                {groupMembers.length > 0 ? groupMembers.filter((filterData, index) => filterData.role === 1).map((data, index) => {
                                  if (data !== null && data !== undefined) {
                          ...
                          <Row>
                            <Col lg={12} md={12} sm={12}>
                              ...
                              <Row>
                                ...
                                >
                                  {meetingAttendeesList.length > 0 ? meetingAttendeesList.map((data, index) => {
                                    console.log("meetingAttendeesListmeetingAttendeesList", data)
                                    return <Row className="mt-4">
                                      <Col lg={12} md={12} sm={12}>
                                        <Row className="d-flex gap-2">
                                          ...
                                          >
                                            <Checkbox
                                              checked={attendees.includes(data.pK_UID) ? true : false}
                                              classNameDiv=""
                                              onChange={() => checkAttendeeBox(data, data.pK_UID, index)}
