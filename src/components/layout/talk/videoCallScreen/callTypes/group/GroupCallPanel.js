...
import GroupCallHeader from "./GroupCallHeader";

/**
 * Group call video surface. The iframe/URL-building/postMessage logic here
 * is identical to OneToOneCallPanel.js on purpose — videoCallNormalPanel.js
 * (the shared original) only ever branched this part of its logic on
 * `isMeeting` (true/false), never on 1:1-vs-group, so there is nothing
 * group-specific to port at the panel level. What's actually different
 * about a group call — the multi-participant roster, accept/decline status,
 * raise-hand, invite-link — lives in GroupCallHeader.js instead.
 * See C:\Users\Administrator\.claude\plans\swift-moseying-sloth.md Phase 3.
 */
const GroupCallPanel = () => {
  ...
  // plain group call (no meeting/presenter forks; see useVideoCallEngine.js
