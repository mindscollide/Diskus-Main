    ...
    try {
      ...
      if (
        ...
        if (
          ...
          // Guarded to skip the person who WAS the unanswered recipient —
          // they never joined this room, so their own local RoomID (built
          // from activeCall/activeRoomID/etc., none of which are
          // meaningfully set for them here) is stale/wrong, making this
          // dispatch fire uselessly (and sometimes twice) on their own
          // client instead of only on the caller's/other participants'.
          const isThisUserTheUnansweredRecipient =
            Number(data.payload.recepientID) === Number(createrID);
          if (
            data.payload.callTypeID === 2 &&
            !isMeetingVideo &&
          ...
          // Rejoin-from-Recent support: when I am the one who missed this
          // group call, surface a narrow, single-purpose signal (separate
          // from the noisy MissedCallCountMqttData, which updates for
          // every kind of missed notification) that the Recent panel can
          // watch to locally show a "Rejoin" option for this exact call,
