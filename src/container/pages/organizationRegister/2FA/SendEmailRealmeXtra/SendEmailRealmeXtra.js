...
const SendEmailRealmeXtra = () => {
  ...
  const onClickRealmeXtra = async (e) => {
    ...
    e.preventDefault()
    if (xtrazoom) {
      let Data = { UserID: JSON.parse(UserID), Device: "POSTMAN", DeviceID: "c", OrganizationID: OrganizationID, isEmail: codeemail, isSMS: codesms, isDevice: xtrazoom, UserDevices: [{ DeviceName: currentDevice.DeviceName, DeviceToken: currentDevice.DeviceRegistrationToken }] }
      await dispatch(sendTwoFacAction(t, navigate, Data))
      await navigate("/verifycodethree", { state: { currentDevice } })
    } else {
      let Data = { UserID: JSON.parse(UserID), Device: "POSTMAN", DeviceID: "c", OrganizationID: OrganizationID, isEmail: codeemail, isSMS: codesms, isDevice: xtrazoom, UserDevices: [] }
      await dispatch(sendTwoFacAction(t, navigate, Data))
    }
  }
  useEffect(() => {
                      ...
                      <Row className="EmailBoxSendRealme">
                        <Col sm={12} md={12} lg={12} className="mt-2">
                          <Row>
                            ...
                            <Col sm={12} md={9} lg={9}>
                              <span className={!xtrazoom ? "SendRealmeXtraZoomColor_active" : "SendRealmeXtraZoomColor"}>
                                SEND NOTIFICATION ON {currentDevice.DeviceName}
                              </span>
                            </Col>
                            <Col sm={12} md={2} lg={2}>
                              <Form.Check type="radio" name="faSendEmailRealmeXtra" value={"SEND NOTIFICATION ON REALME XTRA ZOOM"} onChange={onChangeHandlerSendRealmeXtra1} />
                            </Col>
                          </Row>
                        </Col>
                        <Col sm={12} md={12} lg={12} className="mt-0">
                          <Row className="my-1">
                            ...
                            <Col sm={12} md={9} lg={9}>
                              <span className={!codeemail ? "SendRealmeXtraZoomColor_active" : "SendRealmeXtraZoomColor"}>
                                SEND CODE ON EMAIL
                              </span>
                            </Col>
                            <Col sm={12} md={2} lg={2}>
                              <Form.Check type="radio" name="faSendEmailRealmeXtra" value={"SEND CODE ON EMAIL"} onChange={onChangeHandlerSendRealmeXtra2} />
                            </Col>
                          </Row>
                        </Col>
                        <Col sm={12} md={12} lg={12} className="mt-1">
                          <Row>
                            ...
                            <Col sm={12} md={9} lg={9}>
                              <span className={!codesms ? "SendRealmeXtraZoomColor_active" : "SendRealmeXtraZoomColor"}>
                                SEND CODE ON SMS
                              </span>
                            </Col>
                            <Col sm={12} md={2} lg={2}>
                              <Form.Check type="radio" name="faSendEmailRealmeXtra" value={"SEND CODE ON SMS"} onChange={onChangeHandlerSendRealmeXtra3} />
                            </Col>
                          </Row>
                        </Col>
                      </Row>
