...
const PackageSelection = () => {
  ...
  return (
    <Container>
      ...
      <Row className="mt-3">
        {packageDetail.length > 0 ? packageDetail.map((data, index) => {
          return (<Col sm={12} lg={4} md={4} className="my-2" key={data.pK_SubscriptionPackageID}>
            <Row className="g-4">
              <Col sm={12}>
                <Card className={styles["packagecard"]}>
                  <Row>
                    <Col><img src={PackageCardRibbon} className={styles["packageRibbon"]} /></Col>
                  </Row>
                  <Row>
                    <Col sm={12}>
                      ...
                      >
                        <div className={styles["packagecard_one"]}>
                          <div className={styles["packagecard_pricebox"]}>
                            ...
                            <h4 className={monthlyPackageShow ?
                              `${styles["package_actualPrice"]}`
                              : currentPackageId === data.pK_SubscriptionPackageID ?
                          ...
                          <div className="d-flex">
                            <span
                              className={monthlyPackageShow ? `${styles["spanActive"]}` : monthlyPackageShow && currentPackageId === data.pK_SubscriptionPackageID ? `${styles["spanActive"]}` : `${styles["span"]}`}
                              onClick={() => handleManualPackage(data.pK_SubscriptionPackageID)}
                            >
                              {t("Monthly")}
                            </span>
                            <span
                              className={annualPackageShow && currentPackageId === data.pK_SubscriptionPackageID ? `${styles["spanActive"]}` : `${styles["span"]}`}
                              onClick={() => handleAnnualPackage(data.pK_SubscriptionPackageID)}
                            >
                              {t("Annually")}
                            </span>
                          </div>
                        ...
                        <Col className={annualPackageShow && currentPackageId === data.pK_SubscriptionPackageID
                          ? `${styles["packagecard_two"]}`
                          : ` ${styles["packagecard_two_visible"]}`
                        }
                        >
                          <Col className={styles["packagecard_disoucntprice"]}>
                            <p className={styles["packagecard_disoucntprice_para"]}>
                              {t("PayOnly")}
                            </p>
                            <h4 className="d-flex justify-content-center align-items-center mt-1" >
                              ${data.firstYearDiscountPercentage}/<p>{t("month")}</p>
                            </h4>
                            <p className={styles["packagecard_disoucntprice_para"]}>
                              {t("forFirstYear")}
                            </p>
                  ...
                  <Row>
                    <Col sm={12}>
                      <Button text={" Package"} className={styles["packagecard_btn"]} onClick={() =>handleClickPackage(data.pK_SubscriptionPackageID)}/>
                    </Col>
                  </Row>
                </Card>
              </Col>
