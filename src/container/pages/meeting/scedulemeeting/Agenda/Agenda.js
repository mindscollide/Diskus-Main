...
const Agenda = () => {
  ...
  //   // Find the source and destination main agenda items
  //   const sourceMainAgendaItem = updatedRows.find((row) =>
  //     row.subAgenda.some((item) => item.SubAgendaID === draggableId)
  //   );
  //   const destinationMainAgendaItem = updatedRows.find((row) =>
  //     row.subAgenda.some((item) => item.SubAgendaID === draggableId)
  //   );

  //   if (!sourceMainAgendaItem || !destinationMainAgendaItem) return;

  //   // Find the dragged sub-agenda item
  //   const draggedItem = sourceMainAgendaItem.subAgenda.find(
  //     (item) => item.SubAgendaID === draggableId
  //   );

  //   // Remove the dragged item from the source sub-agenda array
  //   sourceMainAgendaItem.subAgenda.splice(sourceIndex, 1);

  //   // Insert the dragged item into the destination sub-agenda array at the correct index
  //   destinationMainAgendaItem.subAgenda.splice(
  //     destinationIndex,
  //     0,
  //     draggedItem
  //   );
  ...
  //Check Code for subagenda

  // Function to handle reordering of subAgenda items
  const onSubAgendaDragEnd = (result) => {
    if (!result.destination) return; // Dropped outside the list
    const { source, destination } = result;

    // Perform the reorder for all subAgenda items
    const updatedRows = rows.map((row) => {
      // Clone the row and its subAgenda array
      const updatedRow = { ...row };
      updatedRow.subAgenda = [...row.subAgenda];

      // Reorder the subAgenda items within the array
      const [removed] = updatedRow.subAgenda.splice(source.index, 1);
      updatedRow.subAgenda.splice(destination.index, 0, removed);
      ...
      return (
        <>
          <DragDropContext onDragEnd={onSubAgendaDragEnd}>
            <Droppable droppableId="subAgendaDroppable">
              {(provided) => (
