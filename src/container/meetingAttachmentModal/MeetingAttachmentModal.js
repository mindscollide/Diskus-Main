...
import { getMeetingAgendas, meetingModalAttachment, updateAgendaAttachment } from '../../store/actions/VideoChat_actions'
...
const MeetingAttachmentModal = ({ setShow }) => {
    ...
    console.log("fileChangeHandlerfileChangeHandlerfileChangeHandler", agendaAttachments)

    useEffect(() => {
        let MeetingAgendas = VideoChatReducer.MeetingAgendasResponse;
        let AgendasArr = [];
        let AgendaAttachments = [];
        MeetingAgendas.length > 0 && MeetingAgendas.map((agendaData, index) => {
            AgendasArr.push({
            ...
            if (agendaData.meetingAgendaAttachments.length > 0 && agendaData.meetingAgendaAttachments !== null) {
    ...
    const deleteFilefromAttachments = (index) => {
        if (agendaAttachments[index] = index) {
            console.log("agendaAttachments", agendaAttachments)
            agendaAttachments[index].displayAttachmentName = "";
            agendaAttachments[index].originalAttachmentName = "";
        }
        setAgendaAttachment([...agendaAttachments])
        console.log("agendaAttachmentsagendaAttachmentsagendaAttachments", agendaAttachments)
    }
    ...
    return (
        <div>
            <Modal show={VideoChatReducer.isMeetingAttachmentModal} setShow={setShow} ModalTitle={<>
                <Container>
                    <Row className='mt-2'>
                        <Col sm={12} md={12} lg={12} ModalHeaderCloseBtnStyle="mt-2"><h4 ModalHeaderCloseBtnStyle='text-primary fw-bold text-right'><XLg onClick={closeModal} /></h4></Col>
                    </Row>
                </Container>
            </>}
                ModalBody={<Container>
                    <Row>
                        <Col sm={12} md={12} lg={12}><h4 className='text-primary fw-bold text-center border-bottom'>Agenda</h4></Col>
                    </Row>
                    <Row>
                                                        ...
                                                        <>
                                                            <Row>
                                                                ...
                                                                >
                                                                    ...
                                                                    >
                                                                        <Row>
                                                                            {agendaAttachments.length > 0 && agendaAttachments !== null && agendaAttachments.filter((data, index) => data.displayAttachmentName !== "") ?
                                                                                agendaAttachments.map((attachmentData, index) => {
                                                                                    var ext = attachmentData.displayAttachmentName.split(".").pop();
                                                                                    const first = attachmentData.displayAttachmentName.split(" ")[0];
                                                                                    if (first !== "") {
                                                                                        return <>
                                                                ...
                                                                <Col lg={3} md={3} xs={12} className="d-flex align-items-start justify-content-center">
                                                                    <CustomUpload disable={false} change={fileChangeHandler}
                                                            ...
                                                            <Row>
                                                                <Col sm={12} md={12} lg={12} className="d-flex justify-content-end">
                                                                    <Button text="Update" variant="primary" onClick={() => handleUpdate(data.pK_MAID)} />
                                                                </Col>
                                                            </Row>
                                                        </>
                                                    </div>
