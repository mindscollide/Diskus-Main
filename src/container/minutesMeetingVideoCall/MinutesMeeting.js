...
const MinuteMinutesCom = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      assignees.ViewMeetingDetails !== null &&
      assignees.ViewMeetingDetails !== undefined
    ) {
      assignees.ViewMeetingDetails.minutesOfMeeting?.map((data) => {
        minutesArr.push({
  ...
  return (
    <Container className={styles["minutes-container"]}>
      <Row>
        {" "}
        <Col className={styles["minutes_meeting_heading"]}>Meeting Minutes</Col>
      </Row>
      <Row className={styles["minutes-records"]}>
        {isMeetingofMinutes.length > 0
          ? isMeetingofMinutes.map((data, index) => {
              return (
                <>
                  <Col
                    key={data.pK_MOMID}
