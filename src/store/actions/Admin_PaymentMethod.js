        ...
        await axios({
            ...
            if (response.data.responseCode === 417) {
                ...
                if (response.data.responseResult.isExecuted === true) {
                    if (response.data.responseResult.responseMessage.toLowerCase().includes("Admin_AdminServiceManager_GetPaymentMethods_01".toLowerCase())) {
                        dispatch(getPaymentMethod_success(response.data.responseResult.paymentMethod, t("Data-available")))
                    } else if (response.data.responseResult.responseMessage.toLowerCase().includes("Admin_AdminServiceManager_GetPaymentMethods_02".toLowerCase())) {
                        dispatch(getPaymentMethod_fail(t("No-data-available")))
                    } else if (response.data.responseResult.responseMessage.toLowerCase().includes("Admin_AdminServiceManager_GetPaymentMethods_03".toLowerCase())) {
