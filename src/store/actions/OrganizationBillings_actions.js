...
import { invoiceandPaymentHistoryRequestMethod, payOutStandingRequestMethod, getBillingInformationRequestMethod } from '../../commen/apis/Api_config'
...
const getBillingInformation_success = (response, message) => {
    console.log(response, "GET_BLLINGINFORMATION_SUCCESSGET_BLLINGINFORMATION_SUCCESSGET_BLLINGINFORMATION_SUCCESSGET_BLLINGINFORMATION_SUCCESS")
    return {
...
const getBillingInformationapi = (t) => {
    ...
    return ((dispatch) => {
        ...
        form.append("RequestMethod", getBillingInformationRequestMethod.RequestMethod);
        axios({
            method: "post",
            url: getAdminURLs,
            data: form,
            ...
            if (response.data.responseCode === 417) {
                ...
                if (response.data.responseResult.isExecuted === true) {
                    if (response.data.responseResult.responseMessage.toLowerCase().includes("Admin_AdminServiceManager_GetBillingInformation_01".toLowerCase())) {
                        dispatch(getBillingInformation_success(response.data.responseResult.billingInfo, t("Data-available")))
                        console.log(response, "response")
                    } else if (response.data.responseResult.responseMessage.toLowerCase().includes("Admin_AdminServiceManager_GetBillingInformation_02".toLowerCase())) {
            ...
            if (response.data.responseCode === 417) {
                ...
                if (response.data.responseResult.isExecuted === true) {
                    if (response.data.responseResult.responseMessage.toLowerCase().includes("Admin_AdminServiceManager_PayOustanding_01".toLowerCase())) {
                        dispatch(getPayoutstading_success(response.data.responseResult.invoiceDetails, t("Data-available")))
...
const invoiceandpaymenthistory = (t) => {
    ...
    return ((dispatch) => {
        ...
        let form = new FormData();
        form.append("RequestData", JSON.stringify(Data));
        form.append("RequestMethod", invoiceandPaymentHistoryRequestMethod.RequestMethod);
        axios({
            method: "post",
            url: getAdminURLs,
            data: form,
...
export { getBillingInformationapi, getPayoutStandingInformation, invoiceandpaymenthistory }